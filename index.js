...
app.get("/getClasses/:email", (req, res) => {
  const email = req.params.email;

  const sql = "SELECT subjects.*, instructors.firstname, instructors.middlename, instructors.lastname FROM subjects INNER JOIN instructors ON subjects.instructor = instructors.email WHERE subjects.instructor = ? GROUP BY subjects.id"
  DB_CONN.query(sql, email, (err, result) => {
    if (err) {
      console.log(err);
      res.sendStatus(500);
...
app.post("/addClass", (req, res) => {
  ...
  DB_CONN.query(
    ...
    (err1, result) => {
      ...
      console.log(result);
      let classSql = " SELECT subjects.*, instructors.firstname, instructors.middlename, instructors.lastname FROM subjects INNER JOIN instructors ON subjects.instructor = instructors.email WHERE subjects.id = ?  GROUP BY subjects.id"
...
app.post("/addStudent", (req, res) => {
  const firstname = req.body.firstname;
  const yearSection = req.body.courseYearSec;
  ...
  let sql =
    "INSERT INTO students (student_id, firstname, middlename, lastname,gender, year_section) VALUES(?,?,?,?,?,?)";

  DB_CONN.query(
    sql,
    [id, firstname, middlename, lastname,gender, yearSection],
    (err, result) => {
      if (err) {
        console.log(err);
        if (err.errno === 1062)
...
app.post("/getStudents", (req, res) => {
  ...
  const sql = "SELECT students.* FROM students INNER JOIN classes ON classes.student_id = students.student_id WHERE classes.course_number = ?";
  DB_CONN.query(sql, classId, (err, result) => {
    if (err) {
      console.log(err);
      res.sendStatus(500);
...
app.get("/searchStudent/:toSearch", (req, res) => {
    ...
    const sql = 'SELECT CONCAT(firstname , " ", middlename, " ", lastname )as fullname, student_id, year_section FROM `students` WHERE CONCAT(firstname , " ", middlename, " ", lastname) LIKE ?';
...
app.post("/checkAttendace", (req, res) => {
  ...
  let checkDup = "SELECT COUNT(id)as count FROM attendance WHERE date ? AND student_id = ? AND course_id = ?"

    let sql = "SELECT * FROM students INNER JOIN classes on students.student_id = classes.student_id WHERE classes.course_number = ? AND classes.student_id = ?";
...
app.post("/getAttendance",(req, res)=>{
    ...
    const sql = "SELECT date_format(date, '%m-%d-%Y') as date, COUNT(student_id) as present  FROM attendance WHERE course_id = ? GROUP BY date ORDER BY date"
...
app.post("/attendanceDetails",(req, res)=>{
    ...
    const sql = "SELECT students.* FROM students INNER JOIN attendance ON students.student_id = attendance.student_id WHERE date_format(attendance.date, '%m-%d-%Y') = ?"
    DB_CONN.query(sql, date,(err, result)=>{
        if(err){
            console.log(err)
            return res.send({success:false, msg:"Database Error"})
        }
